...
func CheckRepo(root string, cfg Config) ([]Finding, error) {
	...
	fingerprintMap := make(map[string]map[string][]Occurrence)
	fingerprintLines := make(map[string]int)
	...
	for i, ft1 := range allFiles {
		...
		for j := i + 1; j < len(allFiles); j++ {
			...
			findCommonFingerprints(ft1, ft2, cfg, fingerprintMap, fingerprintLines, fingerprintTokens)
		}
	}

	var findings []Finding
	for fp, fileOccs := range fingerprintMap {
		var allOccs []Occurrence
		for _, occs := range fileOccs {
			allOccs = append(allOccs, occs...)
		}
		if len(allOccs) < 2 {
			continue
		}
		var dedupedOccs []Occurrence
		for _, occs := range fileOccs {
			deduped := deduplicateOccurrences(occs)
			dedupedOccs = append(dedupedOccs, deduped...)
		}
		if len(dedupedOccs) < 2 {
			continue
		}
	...
	sort.Slice(findings, func(i, j int) bool {
		...
		if len(findings[i].Occurrences) > 0 && len(findings[j].Occurrences) > 0 {
			if findings[i].Occurrences[0].Path != findings[j].Occurrences[0].Path {
				return findings[i].Occurrences[0].Path < findings[j].Occurrences[0].Path
			}
			return findings[i].Occurrences[0].StartLine < findings[j].Occurrences[0].StartLine
		}
		return false
	})
...
func findCommonFingerprints(ft1, ft2 fileTokens, cfg Config,
	fingerprintMap map[string]map[string][]Occurrence,
	fingerprintLines map[string]int,
	...
	for fp, pos1 := range fp1 {
		...
		if fingerprintMap[fp] == nil {
			fingerprintMap[fp] = make(map[string][]Occurrence)
			startLine := ft1.lines[pos1[0]]
			endLine := ft1.lines[pos1[0]+cfg.MinTokens-1]
			fingerprintLines[fp] = endLine - startLine + 1
		...
		for _, startPos := range pos1 {
			...
			if endLine-startLine+1 >= cfg.MinLines {
				fingerprintMap[fp][ft1.path] = append(fingerprintMap[fp][ft1.path], Occurrence{
					Path: ft1.path, StartLine: startLine, EndLine: endLine,
				})
			}
		}
		for _, startPos := range pos2 {
			...
			if endLine-startLine+1 >= cfg.MinLines {
				fingerprintMap[fp][ft2.path] = append(fingerprintMap[fp][ft2.path], Occurrence{
					Path: ft2.path, StartLine: startLine, EndLine: endLine,
				})
			}
		}
	}
