...
func TestV2PathResolution_DeepestExistingAncestor(t *testing.T) {
	...
	// Build a path: <base>/existing/missing/deeper/missing.json
	leaf := filepath.Join(target, "missing", "deeper", "missing.json")
	got, err := canonicalisePathDetached(leaf)
	if err != nil {
		t.Fatalf("canonicalise: %v", err)
	}
	want := filepath.Clean(filepath.Join(target, "missing", "deeper", "missing.json"))
	if got != want {
		t.Fatalf("deepest-ancestor canonicalisation failed: got=%s want=%s", got, want)
	}
}

// TestV2PathResolution_ParentTraversalCollapses asserts the
// canonical resolver collapses literal ".." segments lexically
// so a path that walks up multiple directories resolves
// cleanly.
func TestV2PathResolution_ParentTraversalCollapses(t *testing.T) {
	// Build: <tmp>/a/b/c with a real directory at <tmp>/a.
	root := t.TempDir()
	if err := os.MkdirAll(filepath.Join(root, "a", "b", "c"), 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	// Traversal: <root>/a/b/c/../../b/c -> <root>/a/b/c
	// after lexical cleanup.
	traversal := filepath.Join(root, "a", "b", "c", "..", "..", "b", "c")
	canon, err := canonicalisePathDetached(traversal)
	if err != nil {
		t.Fatalf("canonicalise: %v", err)
	}
	want := filepath.Clean(filepath.Join(root, "a", "b", "c"))
	if canon != want {
		t.Fatalf("resolver did not collapse .. segments: got=%s want=%s", canon, want)
	}
}
