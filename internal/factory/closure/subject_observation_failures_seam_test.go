...
// subject_observation_failures_seam_test.go owns the fake
// gitClient seam and the row table that the R6-A
// adversarial matrix (TestClosureSubjectObservationFailureMatrix)
// uses. Splitting the seam out of the test file keeps the
// test file under the LLM-friendly 400-line threshold while
// preserving the row-by-row authority/code bindings.
//
// R6-A-CORRECTION02 moved the seam into a dedicated file so
// the failure matrix file holds only the test function. The
// fake and table are the only production-bound test
// surface here; the test file is the public surface that
// the verification step exercises.
