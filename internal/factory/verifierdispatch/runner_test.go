...
func TestAuthorizeAndBindProfileExecutesExactInventory(t *testing.T) {
	verifiers := []registry.Verifier{
		{Name: "v1", Authority: verifierauthority.AuthorityLocalSafe, Lane: registry.VerifierLaneFast, Operations: verifyOnly(), Run: func(root string) []checks.Finding { return nil }, Scope: registry.InvocationGate},
		{Name: "v2", Authority: verifierauthority.AuthorityLocalSafe, Lane: registry.VerifierLaneFast, Operations: verifyOnly(), Run: func(root string) []checks.Finding { return nil }, Scope: registry.InvocationGate},
	}
	d, err := NewDispatcher(verifiers)
	if err != nil {
		t.Fatalf("NewDispatcher: %v", err)
