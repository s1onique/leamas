...
4. `make factorize-canonical` also refuses in editor contexts (depends on `factorize`)
5. No unguarded Make target can start factorize work
6. AGENTS.md and .clinerules/leamas.md no longer recommend routine full-gate/factorize
...
| `CGO_ENABLED=0 go test ./internal/factory/gate/... -run 'Factorize\|Context'` | PASS | 218.27s |
...
- Production-path tests: 4 (all passed)
  - `TestFactorizePublicTargetRefusesInEditorContext`: PASS
  - `TestFactorizeCanonicalRefusesInEditorContext`: PASS
  - `TestFactorizeParallelRefusesInEditorContext`: PASS
  - `TestFactorizeCanonicalWithOverrideAllows`: PASS
